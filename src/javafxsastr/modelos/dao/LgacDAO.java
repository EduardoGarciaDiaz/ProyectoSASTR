...
public class LgacDAO {
     ...
     private static String Consulta_Lgac = "Select idLgac, nombreLgac, descripcionLgac from lgacs";
     private static String Consulta_Lgac_Unica ="Select idLgac, nombreLgac, descripcionLgac from lgacs where idLgac = ?";
    ...
    public ArrayList<Lgac> obtenerInformacionLGCAS() throws DAOException{
        ...
        try {
            PreparedStatement sentencia = ConexionBD.obtenerConexionBD().prepareStatement(Consulta_Lgac);
            ResultSet resultadoConsulta = sentencia.executeQuery();
            if(resultadoConsulta.next()) {
                while(resultadoConsulta.next()) {
                Lgac lgac = new Lgac();
                lgac.setIdLgac(resultadoConsulta.getInt("idLgca"));
                lgac.setNombreLgac(resultadoConsulta.getString("nombreLgca"));
                lgac.setDescripcionLgac(resultadoConsulta.getString("descripcionLgca"));
                }
    ...
    public ArrayList<Lgac> obtenerInformacionLGACsPorCuerpoAcademico(int idCuerpoAcademico) throws DAOException{
        ArrayList<Lgac> lgacConsultados = new ArrayList(); 
        try {
            PreparedStatement sentencia = ConexionBD.obtenerConexionBD().prepareStatement(CONSULTA_LGACS_POR_CUERPO_ACADEMICO);
    ...
    public ArrayList<Lgac> obtenerInformacionLGACsPorAnteproyecto(int idAnteproyecto) throws DAOException{
        ArrayList<Lgac> lgacConsultados = new ArrayList(); 
        try {
            PreparedStatement sentencia = ConexionBD.obtenerConexionBD().prepareStatement(OBTENER_LGACS_POR_ANTEPROYECTO);
    ...
    public Lgac obtenerInformacionLGAC(int idLgac) throws DAOException {
        Lgac lgac = new Lgac();
        try {
            PreparedStatement sentencia = ConexionBD.obtenerConexionBD().prepareStatement(Consulta_Lgac_Unica);
            ResultSet resultadoConsulta = sentencia.executeQuery();   
